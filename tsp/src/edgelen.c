...
static double dtrunc(double);

static int edgelen_nonorm(int i, int j, CCdatagroup* dat), max_edgelen(int i, int j, CCdatagroup* dat),
    man_edgelen(int i, int j, CCdatagroup* dat), euclid_edgelen(int i, int j, CCdatagroup* dat),
    euclid_ceiling_edgelen(int i, int j, CCdatagroup* dat), euclid3d_edgelen(int i, int j, CCdatagroup* dat),
    geographic_edgelen(int i, int j, CCdatagroup* dat), geom_edgelen(int i, int j, CCdatagroup* dat),
    att_edgelen(int i, int j, CCdatagroup* dat), dsjrand_edgelen(int i, int j, CCdatagroup* dat),
    crystal_edgelen(int i, int j, CCdatagroup* dat), matrix_edgelen(int i, int j, CCdatagroup* dat),
    sparse_edgelen(int i, int j, CCdatagroup* dat), user_edgelen(int i, int j, CCdatagroup* dat),
    rhmap1_edgelen(int i, int j, CCdatagroup* dat), rhmap2_edgelen(int i, int j, CCdatagroup* dat),
    rhmap3_edgelen(int i, int j, CCdatagroup* dat), rhmap4_edgelen(int i, int j, CCdatagroup* dat),
    rhmap5_edgelen(int i, int j, CCdatagroup* dat), toroidal_edgelen(int i, int j, CCdatagroup* dat);

static void init_userdat(CCdata_user* userdat), free_userdat(CCdata_user* userdat),
    init_rhdata(CCdata_rhvector* rhdat), free_rhdata(CCdata_rhvector* rhdat);
...
void CCutil_dat_getnorm(CCdatagroup* dat, int* norm) { (*norm) = dat->norm; }
...
static int crystal_edgelen(int i, int j, CCdatagroup* dat) {
    ...
    w1 = dat->y[i] - dat->y[j];
    if (w1 < 0) w1 = -w1;
    if (CRYSTAL_NEEDS_FLIP(w1)) w1 = CRYSTAL_FLIP(w1);
    if (w < w1) w = w1;
    w1 = dat->z[i] - dat->z[j];
    if (w1 < 0) w1 = -w1;
    if (w < w1) w = w1;
...
static int user_edgelen(int i, int j, CCdatagroup* dat) {
    ...
    static const double ibm_xmult[7] = {1062.5, 300.0, 300.0, 250.0, 300.0, 1000.0, 154.6};
    static const double ibm_xadd[7] = {155.0 - 0.01 * 1062.5, 197.5 - 0.05 * 300.0, 212.5 - 0.10 * 300.0,
                                       227.5 - 0.15 * 250.0,  240.5 - 0.20 * 300.0, 255.0 - 0.25 * 1000.0,
                                       305.0 - 0.30 * 154.6};
    static const double ibm_ymult[7] = {1062.5, 450.0, 350.0, 250.0, 300.0, 900.0, 157.7};
    static const double ibm_yadd[7] = {150.0 - 0.01 * 1062.5, 192.5 - 0.05 * 450.0, 215.0 - 0.10 * 350.0,
                                       232.5 - 0.15 * 250.0,  245.5 - 0.20 * 300.0, 250.0 - 0.25 * 900.0,
                                       295.0 - 0.30 * 157.7};
    ...
    if (dw <= 0.01) {
        ...
        dw = dw * ibm_xmult[(int)(dw / 0.05)] + ibm_xadd[(int)(dw / 0.05)];
    }
    if (dw1 < 0.0) dw1 = -dw1;
    dw1 /= 25400.0;
    if (dw1 <= 0.01) {
        ...
        dw1 = dw1 * ibm_ymult[(int)(dw1 / 0.05)] + ibm_yadd[(int)(dw1 / 0.05)];
    }
    if (dw < dw1) dw = dw1;
    return (int)dw;
}
...
static int rhmap3_edgelen(int i, int j, CCdatagroup* dat) {
    ...
    if (second == (char*)NULL) {
        ...
        trans = pow((1.0 - (theta * P)), (double)d) * pow((1.0 - (theta * Q)), (double)a) *
...
static int rhmap5_edgelen(int i, int j, CCdatagroup* dat) {
    ...
    if (cnt == 0)
        ...
        return (int)(10.0 * rhlength * mis) / cnt;
}

...
int CCchunk_localcuts(CCtsp_lpcut_in** clist, int* cutcount, int ncount, int ecount, int* elist, double* x,
...
typedef struct CCchunk_fault_callback {
    int (*func)(CCchunk_graph* chunk, CCchunk_fault* fault, int* finished, void* u_data);
    void* u_data;
...
#define CC_MATVAL_MAX \
    ((((CCmatval)1) << (sizeof(CCmatval) * 8 - 2)) | ((((CCmatval)1) << (sizeof(CCmatval) * 8 - 2)) - 1))
